...
export default function DoctorAppointmentsPage() {
  ...
  const loadAppointments = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await doctorApi.getAppointments(user.id, statusFilter || undefined);
  ...
  const handleAppointmentAction = async (appointmentId: number, action: 'confirm' | 'reject' | 'complete') => {
    if (!user?.id) return;
    ...
    try {
      ...
      switch (action) {
        case 'confirm':
          response = await doctorApi.confirmAppointment(user.id, appointmentId);
          break;
        case 'reject':
          response = await doctorApi.rejectAppointment(user.id, appointmentId);
          break;
        case 'complete':
          response = await doctorApi.completeAppointment(user.id, appointmentId);
          break;
      }
