...
export default function EditMedicalHistoryPage() {
  ...
  const historyId = parseInt(params.id as string);
  ...
  const loadHistory = async () => {
    ...
    try {
      const response = await patientApi.getMedicalHistoryById(user.id, historyId);
      if (response.success && response.data) {
        const history = response.data;
        setValue('condition', history.condition);
        setValue('diagnosisDate', history.diagnosisDate);
        setValue('description', history.description);
        setValue('treatment', history.treatment);
        setValue('medications', history.medications);
        setValue('doctorName', history.doctorName);
        setValue('hospitalName', history.hospitalName);
  ...
  const onSubmit = async (data: MedicalHistoryRequest) => {
    ...
    setIsLoading(true);
    try {
      const response = await patientApi.updateMedicalHistory(user.id, historyId, data);
